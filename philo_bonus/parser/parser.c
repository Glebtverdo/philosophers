...
/*   Updated: 2022/04/30 00:20:17 by vearleen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fill_data(t_vars **result, int argc, char **argv)
{
	...
	(*result)->forks = sem_open(SEM_NAME_FORKS, \
		O_CREAT, 0777, ft_atoi(argv[1]));
	(*result)->is_writing = sem_open(SEM_NAME_IS_WRITING, O_CREAT, 0777, 1);
	sem_unlink(SEM_NAME_FORKS);
	sem_unlink(SEM_NAME_IS_WRITING);
	if ((*result)->is_writing == SEM_FAILED || (*result)->forks == SEM_FAILED)
	{
